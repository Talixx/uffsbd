...
opt_column_list_select: '*' {setColumnSelect(yytext);} | parentesis_open column_list_select parentesis_close;

column_list: column | column ',' column_list;

column_list_select: column_select | column_select ',' column_list_select;

column: OBJECT {setColumnInsert(yytext);};

column_select: OBJECT {setColumnSelect(yytext);};

value_select: OBJECT { setValueColumn(yytext);};

table_select: OBJECT {setTableSelect(yytext);};

attr_select: OBJECT { setValueAttr(yytext);}; | NUMBER { setValueAttr(yytext);}; 

condition: /*optional*/ |'=' {setCondition(yytext);}; 
                        | '>'{setCondition(yytext);};
                        |'<' {setCondition(yytext);}; ;
