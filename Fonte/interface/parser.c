...
rc_parser GLOBAL_PARSER_SELECT;


void setColumnSelect(char **nome) {

    GLOBAL_DATA_SELECT.columnName = realloc(GLOBAL_DATA_SELECT.columnName, (GLOBAL_PARSER_SELECT.col_count+1)*sizeof(char *));

    GLOBAL_DATA_SELECT.columnName[GLOBAL_PARSER_SELECT.col_count] = malloc(sizeof(char)*(strlen(*nome)+1));
    strcpylower(GLOBAL_DATA_SELECT.columnName[GLOBAL_PARSER_SELECT.col_count], *nome);
    GLOBAL_DATA_SELECT.columnName[GLOBAL_PARSER_SELECT.col_count][strlen(*nome)] = '\0';

    GLOBAL_PARSER_SELECT.col_count++;
}


void printValueSelect(char **valor){
    printf("valor -> %s\n",*valor);
}
...
void clearGlobalStructs() {
    ...
    GLOBAL_PARSER_SELECT.mode              = 0;
    GLOBAL_PARSER_SELECT.parentesis        = 0;
    GLOBAL_PARSER_SELECT.noerror           = 1;
    GLOBAL_PARSER_SELECT.col_count         = 0;
    GLOBAL_PARSER_SELECT.val_count         = 0;
    GLOBAL_PARSER_SELECT.step              = 0;
}
